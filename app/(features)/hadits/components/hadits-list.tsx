...
import { Hexagon, Search, BookOpen, X, ChevronDown, Bookmark, Star } from "lucide-react"
import { Hadith, HadithListProps } from "../interfaces/hadits.interface"
...
const HadithList: React.FC<HadithListProps> = ({
  ...
  const filteredHadiths = hadiths.filter((hadith: Hadith) =>
  ...
  useEffect(() => {
    if (isMobile && !selectedHadithId && hadiths.length === 0) {
      setIsMobileExpanded(true)
    }
  }, [isMobile, selectedHadithId, hadiths.length])

  // Keyboard navigation (desktop only)
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (!isMobile && filteredHadiths.length > 0) {
        if (e.key === "ArrowDown") {
          e.preventDefault()
  ...
  }, [filteredHadiths, focusedIndex, isMobile, setSelectedHadithId])
  ...
  const selectedHadith = hadiths.find((hadith) => hadith.no === selectedHadithId)
          ...
          <div className="max-h-60 overflow-y-auto custom-scrollbar">
            {filteredHadiths.length > 0 ? (
              <div ref={listRef} className="divide-y divide-gray-100">
                {filteredHadiths.map((hadith: Hadith, index: number) => (
                  <div
                    key={hadith.no}
                    ...
                    <div className="p-3 flex items-center">
                      ...
                      <div className="flex-1 min-w-0">
                        <h4 className={`font-medium text-sm truncate ${
                          selectedHadithId === hadith.no ? "text-green-800" : "text-gray-800"
                        }`}>
  ...
  return (
    <div className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-lg overflow-hidden h-[435px] xl:h-[80vh] relative z-10 border border-white/20">
      ...
      <div className="overflow-y-auto max-h-[calc(100vh-280px)] custom-scrollbar">
        {filteredHadiths.length > 0 ? (
              ...
              >
                <div className="p-5 flex items-center relative">
                  ...
                  <div className={`relative flex items-center justify-center w-12 h-12 rounded-2xl mr-4 transition-all duration-200 ${
                    ...
                    <span className="absolute text-white font-bold text-xs">{hadith.no}</span>
                    {selectedHadithId === hadith.no && (
          ...
          <div className="flex flex-col items-center justify-center p-12 text-gray-500">
            ...
            <h4 className="font-semibold text-gray-600 mb-2">Tidak ada hadits yang ditemukan</h4>
      ...
      <style jsx>{`
        ...
        .custom-scrollbar::-webkit-scrollbar-track { background: linear-gradient(to bottom, #f8fafc, #f1f5f9); border-radius: 3px; }
        .custom-scrollbar::-webkit-scrollbar-thumb { background: linear-gradient(to bottom, #cbd5e1, #94a3b8); border-radius: 3px; }
        .custom-scrollbar::-webkit-scrollbar-thumb:hover { background: linear-gradient(to bottom, #94a3b8, #64748b); }
      `}</style>
    </div>
  )
}
